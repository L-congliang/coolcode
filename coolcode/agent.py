...
class Agent:
    def __init__(
        ...
        *,
        ...
        self.effective_window = (
            effective_window_override
            if effective_window_override is not None
            else _get_context_window(model) - 20000
        )
        ...
        # COOLCODE_SDK_MAX_RETRIES=0 to isolate our _with_retry in tests.
        _sdk_retries: dict[str, Any] = {}
        _rv = os.environ.get("COOLCODE_SDK_MAX_RETRIES")
    ...
    async def _compact_conversation(self) -> None:
        before_chars = self._context_size_chars()
        before_input_tokens = self.total_input_tokens
        self._emit_event(
            "compression_start",
            tier="autocompact",
        ...
        self._emit_event(
            ...
            tier="autocompact",
            before_chars=before_chars,
            after_chars=self._context_size_chars(),
            summary_input_tokens=self.total_input_tokens - before_input_tokens,
        )
    ...
    async def _compact_anthropic(self) -> None:
        ...
        self.total_input_tokens += getattr(summary_resp.usage, "input_tokens", 0) or 0
        self.total_output_tokens += getattr(summary_resp.usage, "output_tokens", 0) or 0
        self._anthropic_messages = [
    ...
    async def _compact_openai(self) -> None:
        ...
        if summary_resp.usage:
            self.total_input_tokens += getattr(summary_resp.usage, "prompt_tokens", 0) or 0
            self.total_output_tokens += getattr(summary_resp.usage, "completion_tokens", 0) or 0
        self._openai_messages = [
            system_msg,
            {"role": "user", "content": f"[Previous conversation summary]\n{summary_text}"},
            {"role": "assistant", "content": "Understood. I have the context from our previous conversation. How can I continue helping?"},
    ...
    def _persist_large_result(self, tool_name: str, result: str) -> str:
        ...
        if len(result.encode()) <= THRESHOLD:
            return result
        d = Path.home() / ".coolcode" / "tool-results"
...
IMPORTANT: When your plan is complete, you MUST call exit_plan_mode. Do NOT ask the user to approve — exit_plan_mode handles that."""
    ...
    def _consume_memory_prefetch_if_ready(self, messages: list) -> None:
        ...
        try:
            ...
            if not memories:
                self._emit_event("memory_retrieve", count=0, paths=[])
                return
            self._emit_event(
                "memory_retrieve",
                count=len(memories),
    ...
    def _start_memory_prefetch_for_turn(self, user_message: str, messages: list) -> None:
        ...
        if self.is_sub_agent or not self.memory_enabled:
            return
        ...
        if sq:
            ...
            if self._memory_prefetch:
                self._emit_event(
                    ...
                    query=user_message,
                    surfaced_count=len(self._already_surfaced_memories),
                )
    ...
    async def _chat_anthropic(self, user_message: str) -> None:
        ...
        while True:
            ...
            for tu in tool_uses:
                ...
                self._emit_event(
                    "permission_decision", tool=tu.name, input=inp,
                    action=perm.get("action"), message=perm.get("message", ""),
                )
                if perm["action"] == "deny":
                ...
                if perm["action"] == "confirm" and perm.get("message"):
                    ...
                    if not cacheable or perm["message"] not in self._confirmed_paths:
                        ...
                        self._emit_event(
                            "permission_confirmation", tool=tu.name,
                            input=inp, allowed=confirmed,
                        )
                        if not confirmed:
    ...
    async def _chat_openai(self, user_message: str) -> None:
        ...
        while True:
            ...
            for tc in tool_calls:
                ...
                self._emit_event(
                    "permission_decision", tool=fn_name, input=inp,
                    action=perm.get("action"), message=perm.get("message", ""),
                )
                if perm["action"] == "deny":
                    print_info(f"Denied: {perm.get('message', '')}")
                ...
                if perm["action"] == "confirm" and perm.get("message"):
                    ...
                    if not cacheable or perm["message"] not in self._confirmed_paths:
                        confirmed = await self._confirm_dangerous(perm["message"])
                        self._emit_event(
                            "permission_confirmation", tool=fn_name,
                            input=inp, allowed=confirmed,
                        )
                        if not confirmed:
