...
def _web_fetch(inp: dict) -> str:
    ...
    req = urllib.request.Request(url, headers={"User-Agent": "coolcode/1.0"})
    try:
...
def _matches_rule(rule: dict, tool_name: str, inp: dict) -> bool:
    ...
    if "file_path" in inp:
        ...
        pattern_path = Path(pattern[:-1] if pattern.endswith("*") else pattern)
        if not value_path.is_absolute():
            value_path = Path.cwd() / value_path
        if not pattern_path.is_absolute():
            pattern_path = Path.cwd() / pattern_path
        normalized_value = os.path.normcase(str(value_path.resolve(strict=False)))
        normalized_pattern = os.path.normcase(str(pattern_path.resolve(strict=False)))
        if pattern.endswith("*"):
...
def _tool_references_denied_path(tool_name: str, inp: dict, rule: dict) -> bool:
    if rule.get("tool") not in {"read_file", "write_file", "edit_file"}:
        return False
    if tool_name not in {"run_shell", "grep_search", "agent"}:
        return False
    ...
    if tool_name == "run_shell":
        ...
        value = str(inp.get("path", ""))
    else:
        value = str(inp.get("prompt", ""))
...
def _check_permission_rules(tool_name: str, inp: dict) -> str | None:
    ...
    if any(_tool_references_denied_path(tool_name, inp, rule) for rule in rules["deny"]):
        return "deny"
